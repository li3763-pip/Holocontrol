...
function renderHome(){
  ...
  document.getElementById('inv-s3').textContent=inv.s3;
...
function renderPerf(){
  ...
  document.getElementById('p-folios').innerHTML=`
    ...
    <div class="irow"><div class="irow-l">Hologr. S3</div><div class="irow-v mono" style="font-size:10px">${SESSION.fs3||'—'}</div></div>`;
...
function computeHoloFolios(){
  ...
  const usedS3=_countUsedInRegistros('S3');
  ...
  const rS3=SESSION?_parseRange(SESSION.fs3):null;
  const rUVA=SESSION?_parseRange(SESSION.fuva):null;
  let batchS1=0, batchS2=0, batchS3=0, batchUVA=0;
  for(let i=0;i<n;i++){
    ...
    if(tipo==='S1'&&rS1){
      ...
      profEl.value=rS3.prefix+String(rS3.start+usedS3+batchS3).padStart(rS3.padLen,'0');
      batchS3++;
    } else {
      profEl.value='';
    }
    // UVA folio — se asigna cuando hay tipo válido (S1, S2 o S3)
    if((tipo==='S1'||tipo==='S2'||tipo==='S3')&&rUVA){
      ...
      <div class="fgrid" style="margin-bottom:6px">
        <div class="fld" style="margin-bottom:0">
          ...
          <select class="fi" id="ht-${i}" onchange="computeHoloFolios()" style="padding:9px 10px;font-size:12px">
            ...
            <option value="S3">S3 — 3er Semestre</option>
...
function guardarRegistro(){
  ...
  instrumentos.forEach(inst=>{
    ...
    if(inst.holoTipo==='S3') SESSION.inv.s3=Math.max(0,SESSION.inv.s3-1);
